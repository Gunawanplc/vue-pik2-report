...
const { data, isConnected, sendMessage } = useWebSocket("/ws/mainRT");
...
const series = ref([]);
  ...
  (wsPayload) => {
    ...
    { immediate: true } // Tambahkan immediate: true
  );
  ...
  function mapWsToGroupCharts(wsData) {
    ...
    return Object.keys(wsData)
      .filter(k => k.startsWith("IdRT"))
      .map(key => {
        const arr = wsData[key];
        if (!arr) return null;
        
        const data = arr;
        const oldData = arr.old;
        const newData = arr.new;

        const Kategory = new Date(oldData.time).toLocaleTimeString("fr-fr").substring(0,5) +"-"+new Date(newData.time).toLocaleTimeString("fr-fr").substring(0,5)
        const old_time = arr.old_orderbook.time
        const old_change = arr.old_orderbook.change
        const old_price = arr.old_orderbook.lastprice
        const new_time = arr.new_orderbook.time
        const new_change = arr.new_orderbook.change
        const new_price = arr.new_orderbook.lastprice
